...
import { getUserData, getChatData, sendMessage } from '../actions';
...
function HomeChat() {
      ...
      */
      socket.on('chats', function (data) {
        if (data.action === 'create') {
          getChatData().then((res) => dispatch(setChatsData(res.data.chats)));
        } else if (data.action === 'SentNewMessage') {
          getChatData().then((res) => dispatch(setChatsData(res.data.chats)));
        } else if (data.action === 'ReceivedNewMessage') {
          getChatData().then((res) => dispatch(setChatsData(res.data.chats)));
          ...
          Swal.fire({
            icon: 'error',
            title: 'Se ha producido un error al enviar o recibir el mensaje',
            toast: true,
            position: 'top-end',
            confirmButtonColor: '#36dd81'
          });
        }
      });
      return () => {
  ...
  const getChatsData = () => {
    /* : 
      Get all chats data 
    */
    getChatData().then((res) => dispatch(setChatsData(res.data)));
  };
